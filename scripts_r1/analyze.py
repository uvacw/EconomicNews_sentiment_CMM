...
def generate_scatterplots(data, field, tool_order, folder, standardization=False):
    from scipy.stats.mstats import zscore
    ...
    if standardization:
        ...
        for combi in combinations(colnames, r=2):
            x, y = combi
            fit = plt.figure()
            sns_plot = sns.scatterplot(x=x, y=y, data=data_std)
            fig = sns_plot.get_figure()
            fig.savefig(folder+'/'+x+'_'+y+'.png')
            sns_plot = sns_plot.clear()

    else:
        print(data.describe().transpose())
        for combi in combinations(cols, r=2):
            x, y = combi
            sns_plot = sns.scatterplot(x=x, y=y, data=data)
            fig = sns_plot.get_figure()
            fig.savefig(folder+'/'+x+'_'+y+'.png')
            sns_plot = sns_plot.clear()
